...
class inflow_outflow():
    ...
    def create_inflow_outflow(self):     
        for file in self.files:
            ...
            trajectory = self.process_long_lati_one_trajectory_rome(trajectory, inflow_outflow_raw_data_pathname)
    ...
    def process_long_lati_one_trajectory_rome(self, trajectory, inflow_outflow_raw_data_pathname):
        ...
        for k, item in enumerate(trajectory):
            # 如果超过了我们统计的经纬度范围，跳过
            # item : [0(id),1(timestamps), 2(lati), 3(long)]
            item[1] = int(item[1][:-3])
            item[2] = float(item[2])
            item[3] = float(item[3])
            ...
            if item[3] < self.begin_long or item[3] > self.end_long:
                counter_long_break += 1
                area = (-1, -1)
            elif item[2] < self.begin_lati or item[2] > self.end_lati:
                counter_lati_break += 1
                area = (-1, -1)
            else:
                # 计算这个点是哪个区域的
                i = (item[3] - self.begin_long) // long_interval
                j = (item[2] - self.begin_lati) // lati_interval
                i = int(i)
                j = int(j)
                area = (i, j)    # 这次的区域
                counter += 1
...
class flow(object):
    ...
    def get_trajectory_rome(self, pathname):
        with open(pathname, "r") as f:
            trajectory = []
            for item_str in f:
                item_str = item_str.strip("\n")
...
def hello(begin_long,end_long,begin_lati,end_lati,time_interval,m,n,nb_flows):
    h5_file_name = "./info_rome/TaxiRM_" + str(begin_long) + "_" + str(end_long) + "_" + str(begin_lati) + "_" + str(end_lati) + "_" + str(time_interval) + "_" + str(m) + "_" + str(n) + "_" + str(nb_flows) + "_count.h5"
    ...
    parent_dir = "/home/linc-3/one_storage/data.bak/rome_data/single/"
    ...
    with open("./info_rome/" + str(time_interval) + "_" + str(begin_long) + "_" + str(end_long) + "_" + str(begin_lati) + "_" + str(end_lati) + "_" + str(m) + "_" + str(n) + ".files","w") as f:
    ...
    for f in flow.files:
        ...
        #print(f)
        trajectory = two.get_trajectory_rome(f)
...
class deal_h5:
    ...
    def deal(self):
        ...
        self.count = np.zeros((int(self.max) + 1), dtype='i')
...
if __name__ == "__main__":
    ...
    name = hello(begin_long, end_long, begin_lati, end_lati, time_interval, m, n, nb_flows)
    h5 = deal_h5(name)
    h5.deal()
    print("max:", h5.max, h5.sum)

    # maxhhh = 0
    # for i in np.arange(begin_lati, 41.82, 0.01):
    #     for j in np.arange(41.86, end_lati, 0.01):
    #         for l in np.arange(begin_long, 12.5, 0.01):
    #             for k in np.arange(12.5, end_long, 0.01):
    #                 name = hello(l, k, i, j, time_interval, m, n, nb_flows)
    #
    #                 h5 = deal_h5(name)
    #                 h5.deal()
    #                 print(i, ", ", j, ",", l, ", ", k, "\n")
